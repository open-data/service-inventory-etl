...
def process_performance_calculation(total_volume, volume_meeting_target):
    try:
        return round((volume_meeting_target / total_volume)*100,0)
    except:
        return 'ND'
...
def run_services_transformation(extract_file, output_path, output_file):
    ...
    external_internal_lookup = {
        ...
        'Internal Cluster':'cluster intern'
    }
   ...
   # Create service type column
    service_type_lookup = {
        ...
        'Information':'info',
        'Legal':'legal',
        'Management and Oversight':'mgnt',
        'Material Services':'mat',
        'Regulatory Compliance and Enforcement':'reg',
        'Resources':'res',
        'Real Property Management':'prop',
        'Rule Making':'rule',
        'Care, Education, Recreation':'cer',
        'Agreements, Permissions, Inspections, Rulings':'apir',
        'Penalties, Protection, Intervention':'ppi',
        'Legislation, Regulation, Policy':'lrp',
        'Material Management':'mat'
    }
   ...
   # Create special designations column
    ...
    # Create service recipient type column
    service_receipient_type_lookup = {
        'Client-based Services':'client',
        'Societal-based Service':'society',
        'Client-based Services<>Societal-based Service':'client, society',
        '' : ''
    }
    df_services['service_recipient_type'] = df_services['Service Recipient Type (English)'].apply(lambda x: process_lookup_map(x, service_receipient_type_lookup, ''))
    select_columns.append('service_recipient_type')
    ...
    # Create How is the service assessed for accessibility? (English) column
    accessibility_lookup = {
        'Other accessibility criteria':'other/autre',
        'WCAG 2.0 AA standard':'WCAG 2.0 AA',
        'TBS/WCAG plain language requirement':'TBS/WCAG plain language',
        'WCAG 2.1 AA standard':'WCAG 2.1 AA',
        'Not Applicable':'na',
        'EN 301-549 (2018) standard':'EN 301-549 (2018)',
        'Persons with disability involved in the assessment or testing':'Persons with disability involved in the assessment or testing',
        'Not assessed':'not assessed',
        '':''
    }
    df_services['assessed_for_accessibility'] = df_services['How is the service assessed for accessibility? (English)'].apply(lambda x: process_lookup_map(x, accessibility_lookup, ''))
    select_columns.append('assessed_for_accessibility')

    # Create year of last service review column
    df_services['year_of_last_service_review'] = df_services['Year of last service review']
    select_columns.append('year_of_last_service_review')

    # Create year of last service review based on client feedback column
    df_services['year_of_last_service_improvement_based_on_client_feedback'] = df_services['Year of last service improvement based on client feedback']
    select_columns.append('year_of_last_service_improvement_based_on_client_feedback')
    ...
    # Create telephone_enquries column
    df_services['telephone_enquiries'] = df_services['Telephone Enquiries'].apply(lambda x: process_num_or_ND(x))
    select_columns.append('telephone_enquiries')
    ...
    # Create email_applications column
    df_services['email_applications'] = df_services['Email Applications'].apply(lambda x: process_num_or_ND(x))
    select_columns.append('email_applications')

    # Create fax_applications column
    df_services['fax_applications'] = df_services['Fax Applications'].apply(lambda x: process_num_or_ND(x))
    select_columns.append('fax_applications')
